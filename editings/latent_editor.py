...
class LatentEditor(object):
    ...
    def apply_ganspace(self, latent, ganspace_pca, edit_directions):
        ...
        return self._latents_to_image(edit_latents.cuda())
    ...
    def _latents_to_image(self, latents, randomize_noise=False):
        with torch.no_grad():
            images, _ = self.generator([latents], randomize_noise=randomize_noise, input_is_latent=True)
            if self.is_cars:
